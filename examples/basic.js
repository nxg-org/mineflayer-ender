...
const enderPearling = require( "../lib/index");
const{ Vec3 }= require("vec3")
const utilPlugin = require("@nxg-org/mineflayer-util-plugin")
const {pathfinder, goals} = require( "mineflayer-pathfinder")
const{ promisify }= require("util")
const { mkdirSync, writeFileSync } = require("fs")
const path = require("path")
const { EnderShotFactory } = require("../lib/enderShotFactory")
...
bot.loadPlugin(enderPearling.default);
bot.loadPlugin(pathfinder);
let pearlThrown = false

bot.prependListener('inject_allowed', () => {
    const original = bot._client.registerChannel.bind(bot._client)
    bot._client.registerChannel = (name, type) => {
        if (name !== 'MC|Brand') {
    ...
    // bot.registry = md(bot._client.version)
})
...
function resolveLogPath(logArg) {
...
function writeShotLog(logPath, pearlTarget, pearlBlock, face, shotInfo) {
